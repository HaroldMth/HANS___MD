...
async function handler(conn, mek, m) {
  ...
  try {
    ...
    try {
      const report = `
⚠️ *COMMAND CRASH REPORT* ⚠️

*Command:* \`${cmd.pattern}\`
*User:* @${resolvedSenderNumber || "unknown"}
*Group:* ${isGroup ? from : "DM"}
...
*Error:*
\`\`\`
${(err.stack || err.message || String(err)).substring(0, 3800)}
\`\`\`
      `.trim();
      await sendTG(report);
    } catch (tgErr) {
      console.error("[CMD CRASH TG REPORT] Failed:", tgErr.message);
    }
  }
}
