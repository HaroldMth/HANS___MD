...
function buildLidMap(conn) {
  ...
  // Manual LID mappings for known devs (until they message the bot)
  const manualMappings = {
    "215057921224886": "237694668970", // Dev 1
    "244375770640458": "237680260772", // Dev 2  
    "94468325777519": "238696900612",  // Dev 3
    "77099645800455": "237696900612",  // Additional dev
  };
  
  for (const [lidNum, pnNum] of Object.entries(manualMappings)) {
    if (lidNum && pnNum) map.set(lidNum, pnNum);
  }
...
async function handler(conn, mek, m) {
  ...
  const resolvedSenderNumber = lidMap.get(normalizeId(senderNumberRaw)) || normalizeId(senderPnJid || senderNumberRaw);
  ...
  const DEV_NUMBERS = ["237696900612", "237694668970", "237680260772"];
  const isDev = DEV_NUMBERS.includes(resolvedSenderNumber);
  const isOwner = config.OWNER_NUMBER.includes(resolvedSenderNumber) || isDev;
